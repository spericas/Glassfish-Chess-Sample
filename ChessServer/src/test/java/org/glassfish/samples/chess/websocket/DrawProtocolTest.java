...
/**
 * DrawProtocolTest class.
 *
 * @author Santiago.Pericas-Geertsen@oracle.com
 */
public class DrawProtocolTest {
    ...
    /**
     * Example of W requesting a draw and B accepting it.
     *
     * (1) W --- SendAction[DRAW_REQUEST] ---> Server
     * (2) B <-- SendAction[DRAW_REQUEST] --- Server
     * (3) B --- SendActionRsp[DRAW_REQUEST_ACCEPTED] ---> Server
     * (4) W <-- SendActionRsp[DRAW_REQUEST_ACCEPTED] --- Server
     */
    @Test
                            ...
                            public void onMessage(Message message) {
                                try {
                                    if (message instanceof CreateGameRsp) {
                                        ...
                                        sa.setType(SendAction.Type.DRAW_REQUEST);
                                        sa.setColor(Color.W);
                                        session.getBasicRemote().sendObject(sa);
                                        System.out.println("W sent SendAction[DRAW_REQUEST]");
                                    } else if (message instanceof SendActionRsp) {
                                        SendActionRsp sar = (SendActionRsp) message;
                                        assertEquals(SendActionRsp.Type.DRAW_ACCEPTED, sar.getType());
                                        assertEquals(Color.W, sar.getColor());
                                        System.out.println("W received SendActionRsp[DRAW_ACCEPTED]");
                                        third.countDown();
                                    }
                            ...
                            public void onMessage(Message message) {
                                try {
                                    if (message instanceof SendAction) {
                                        ...
                                        assertEquals(SendAction.Type.DRAW_REQUEST, sa.getType());
                                        assertEquals(Color.W, sa.getColor());
                                        System.out.println("B received SendAction[DRAW_REQUEST]");
                                        SendActionRsp sar = new SendActionRsp();
                                        sar.setGameId(message.getGameId());
                                        sar.setType(SendActionRsp.Type.DRAW_ACCEPTED);
                                        sar.setColor(message.getColor());
                                        session.getBasicRemote().sendObject(sar);
                                        System.out.println("B sent SendActionRsp[DRAW_ACCEPTED]");
                                        third.countDown();
                                    }
